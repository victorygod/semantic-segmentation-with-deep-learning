...
void getICWeight(const Mat &contourMat, Eigen::SparseMatrix<float> &WIC, int step = 30) {
	for (int r = 0; r < contourMat.rows; r++) {
		const float* colData = contourMat.ptr<float>(r);
		for (int c = 0; c < contourMat.cols; c++) {
			int bottomBound = min(r + step, contourMat.rows);
			int upperBound = max(r - step, 0);
			int sideBound = min(c + step, contourMat.cols);
			for (int i = 0; i < step; i++) {
				int deltaRow[4] = { r - upperBound, i, -i, bottomBound - r };
				int deltaCol[4] = { i, sideBound - c, sideBound - c, i };
				...
				for (int d = 0; d < 4; d++) {
					...
					for (int row = 0; row < deltaRow[d]; row++) {
						y.y = r + 1;
						while (y.y == round(y.x / k[d]) && y.y <= sideBound) {
							if (contourMat.at<float>(y) > maxCon) {
								maxCon = contourMat.at<float>(y);
								maxPoint = y;

							}
							float Pcon = 1 - maxCon;
							int row_wic = contourMat.rows*r + contourMat.cols*c - 1;
							int col_wic = contourMat.rows*y.y + contourMat.cols*y.y - 1;
							WIC.insert(row_wic, col_wic) = Pcon;
							y.y++;
...
void getWeight(int i_row, int i_col, int j_row, int j_col, float step, const Mat &contourMat, Eigen::SparseMatrix<float> &WIC) {//获得两点之间的权重，额，不知道怎么写，还没写好
	float k = (j_row - i_row) / (j_col - i_col);
	int rs = min(i_row, j_row);
	int re = max(i_row, j_row);
	int cs = min(i_col, j_col);
	int ce = max(i_col, j_col);
	Point2f maxPoint(cs, rs);
	float maxCon=-10.0;
	for (float m = cs; m < ce; m += 4*step) {
		int p_x = floorf(m); 
		int p_y = round(m*k);
		const float* colData = contourMat.ptr<float>(p_x);
		const float pVal = colData[p_y];
		if (pVal >= maxCon) {
			maxCon = pVal;
			maxPoint.x = p_x;
			maxPoint.y = p_y;
...
int main() {
	...
	Mat icMat(u.rows, u.cols, CV_32FC1);
	contourMat(channels[2], icMat);//得轮廓图dst

	const int pix = u.cols*u.rows;//总像素个数
	Eigen::SparseMatrix<float> sparse_weight(pix, pix);//稀疏权重图W
	Eigen::SparseMatrix<float> ic_weight(pix, pix);
